...
export class Camera {
  ...
  update() {
    ...
    mat4.lookAt(this.view, this.view_pos, this.target, [0, 1, 0]);
  ...
  rotate(dx, dy) {
    ...
    vec3.sub(xz_plane_direction, this.look_position, this.view_pos);
    ...
    vec3.transformMat4(this.view_pos, this.view_pos, rotation);
  }

  zoom(delta) {
    let diff = vec3.create();
    vec3.sub(diff, this.view_pos, this.look_position);
    vec3.scale(diff, diff, 1.0+delta);

    vec3.add(this.view_pos, this.look_position, diff);
  }
