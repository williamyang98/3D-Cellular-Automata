...
export class VoxelGridPerformance {
  ...
  create_buffers() {
    ...
    // x, y, z, n1, n2, n3
    this.vertex_data = new Float32Array(this.cell_count * this.total_vertices * 3);
    this.index_data = new Uint32Array(this.cell_count * this.total_triangles * 3); 
  }
    ...
    for (let z = 0; z < this.size[2]; z++) {
      for (let y = 0; y < this.size[1]; y++) {
        for (let x = 0; x < this.size[0]; x++) {
          ...
          // add vertex data
          let vertex_data = cube_optimized.vertex_data(left, right, front, back, top, bottom);
          let vertex_buffer_offset = current_cell*this.total_vertices*3;
          for (let i = 0; i < this.total_vertices*3; i++) {
            this.vertex_data[vertex_buffer_offset+i] = vertex_data[i];
          }

          // add index data
          let index_data = cube_optimized.index_data.map(index_mapping);
          let index_buffer_offset = current_cell*this.total_triangles*3;
          for (let i = 0; i < this.total_triangles*3; i++) {
            this.index_data[index_buffer_offset + i] = index_data[i];
          }
